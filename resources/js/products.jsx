...
const Row = React.memo(function Row({ p, onEdit }) {
  ...
  return (
    <tr className={isLow ? "row-low" : ""} data-low={isLow ? "1" : "0"} title={isLow ? "Produto com baixo estoque" : undefined}>
      ...
      <td className="action">
        <a className="link" href="#novo-produto" onClick={(e) => onEdit(p, e)}>
          <VscEdit size={20} /> Editar
        </a>
      </td>
    </tr>
  );
...
function Products() {
  ...
  // cache de respostas por chave (q|low)
  const cacheRef = useRef(new Map());
  // controller ativo para abortar buscas anteriores
  ...
  const onFieldChange = useCallback((key) => (e) => {
    setForm((prev) => ({ ...prev, [key]: e.target.value }));
  }, []);

  const fetchList = useCallback(async (params = {}) => {
    const qParam = params.q ?? q;
    const lowParam = params.low ?? low;
    const key = `${qParam}|${lowParam}`;
    ...
    // abort a requisição anterior (se houver)
  ...
  const onToggleLow = useCallback(() => {
    const next = low === 1 ? 0 : 1;
    setLow(next);
    setQ("");
    fetchList({ low: next, q: "" });
  }, [low, fetchList]);
  ...
  useEffect(() => {
    const onHash = () => { /* Mantém estado do form ao fechar pelo hash */ };
        ...
        <section className="panel" style={{ marginTop: 16 }}>
          ...
          <section id="novo-produto" className="modal" aria-labelledby="titulo-modal" aria-modal="true" role="dialog">
            ...
            <div className="modal__panel">
              ...
              <h2 id="titulo-modal" className="modal__title">{mode === "edit" ? "Editar produto" : "Adicionar produto"}</h2>
              ...
              <form method="post" action={mode === "edit" ? `/products/${editingId}` : "/products"}>
                ...
                <div className="form__actions">
                  {mode === "edit" && (
                    <button type="submit" className="btn danger" form="product-delete-form" onClick={(e) => { if (!confirm("Tem certeza que deseja excluir este produto?")) e.preventDefault(); }} title="Excluir produto"><VscTrash size={20} /> Excluir</button>
                  )}
